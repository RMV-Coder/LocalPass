...
| **lp-sync** | Op-log sync: ingest verification (signature/seq/hash-chain/Lamport), deterministic total-merge, file-based log shipping, offline device-pairing groundwork. | `engine::{setup,push,pull,status,adopt,share_vault_to_device,import_shared_key}`, `verify::verify_batch`, `merge::materialize`, `shipping::SyncDir`, `identity::DeviceIdentity::{to_export_string,from_export_string,fingerprint}` | `lp-vault`, `lp-crypto` | AGPL-3.0 |
