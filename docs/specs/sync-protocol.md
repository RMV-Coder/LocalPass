...
serialization of these fields, in this exact order:

```
Op (signed region = fields 1..10, signature = field 11):
...
- **Canonical serialization:** fixed-order, fixed-width integers (little-endian),
  length-prefixed `payload_env` (u32 length). Deterministic so the signed bytes
  are reproducible on any device. (JSON is *not* used on the wire; the item
  *payload inside* `payload_env` is the canonical-JSON body of
  `vault-format.md` §4.)
...
- **Sign-after-encrypt (stated, with rationale):** the Ed25519 signature covers
  the **ciphertext** `payload_env`, not the plaintext. This binds the signature
  to exactly the bytes that traverse the untrusted channel, so a relay or file
  host cannot substitute, truncate, or splice ciphertext without invalidating
  the signature (PRD §8 T5). Because the payload is authenticated *twice* (the
  AEAD Poly1305 tag under VaultKey for confidential integrity, and Ed25519 for
...
- On ingesting a remote op with clock `L`:
  ...
  authoring). This yields the standard happens-before partial order; the total
  order for merge is `(lamport, device_id)` (§4).
...
"Concurrent" strictly means neither op is in the other's `prev_hash`/lamport
causal past. Edit-wins over delete is the one place the pure LWW comparator is
overridden — and it is overridden *toward preservation*, never toward loss.
...
prev_hash(op) = BLAKE3( canonical_bytes(previous op by SAME device, fields 1..11) )
prev_hash(first op by a device) = BLAKE3("localpass/v1/chain-genesis" | vault_id | device_id)
```

Note the chain covers fields **1..11** (including the signature) of the prior op,
so the chain commits to the signed *and* signature bytes — a peer cannot swap a
validly-signed but different prior op.

**On ingest, a peer verifies for each incoming op:**

1. Ed25519 `signature` valid over fields 1..10 under `device_id`'s known public
   key (from `peer_devices`, `vault-format.md` §2). Unknown device → reject.
...
- `<device_id>-<seq_lo>-<seq_hi>.oplog` body = length-prefixed concatenation of
  canonical op bytes (fields 1..11 each), same encoding as §1. The ops are
  already E2EE (`payload_env`) and signed; the file adds no security, only
  framing.
...
6. The Lamport total order `(lamport, device_id, op_id)` is the sole tiebreak
   basis for LWW; it is deterministic and total (§4.1).
